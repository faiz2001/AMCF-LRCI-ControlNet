...
class ControlledUnetModel(UNetModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ctrl_channels = [320, 320, 320, 320, 640, 640, 640,
    ...
    def forward(self, x, timesteps=None, context=None, control=None,
        ...
        # Scheduler weight — scalar → (1,1,1,1) for broadcasting
        weight = self.control_scheduler.weight(timesteps[0]).to(x.device)
        weight = weight.view(1, 1, 1, 1)

        # Apply middle control (index -1)
        if control is not None:
            mid_ctrl = control[-1]
            fused    = self.fusion_modules[-1]([mid_ctrl])
            delta    = self.lrci_adapters[-1](fused)
            h        = h + weight * delta

        # Output blocks
        ctrl_idx = len(control) - 2 if control is not None else 0
        ...
        for i, module in enumerate(self.output_blocks):
            if only_mid_control or control is None:
                ...
                skip     = hs.pop()
                ctrl     = control[ctrl_idx]
                fused    = self.fusion_modules[ctrl_idx]([ctrl])
...
class ControlLDM(LatentDiffusion):
    ...
    @torch.no_grad()
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50,
                   ddim_eta=0.0, return_keys=None, quantize_denoised=True,
                   inpaint=True, plot_denoise_rows=False,
